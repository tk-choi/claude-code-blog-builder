...
## 9. 각도 일치 확인
- `metadata.json`의 `angle_summary` 필드 확인 (있을 경우)
- `post.md` 도입부가 해당 각도의 핵심 주장을 다루는지 검토
- 처음 의도한 각도와 다른 방향으로 작성됐으면 REVIEW

## 최종 보고
사용자에게 체크리스트 결과를 표로 제시하고, FAIL/REVIEW 항목만 수정 방향 제안.
모든 항목 PASS면 "발행 준비 완료 — Tistory 에디터에서 수동 업로드하세요" 안내.
